...
app.config['DETECTIONS_DIR'] = os.path.join(os.getcwd(), CONFIG['save_dir'])  # Absolute path to detections
    ...
    if request.method == 'POST':
        ...
        if form_id == 'video-source-form':
            ...
            if video_source == 'camera':
                if CONFIG['use_test_video'] or CONFIG['camera_source'] != int(request.form.get('camera_id', 0)):
                    CONFIG['use_test_video'] = False
                    CONFIG['camera_source'] = int(request.form.get('camera_id', 0))
                    restart_required = True
            elif video_source == 'test_video':
                if not CONFIG['use_test_video'] or CONFIG['test_video_path'] != request.form.get('test_video'):
        ...
        # Handle model settings form
            ...
            # If using custom model
                ...
                # Update custom model type if changed
                custom_model_type = request.form.get('custom_model_type', 'torchscript')
                if CONFIG['custom_model_type'] != custom_model_type:
                    CONFIG['custom_model_type'] = custom_model_type
                    restart_required = True
        ...
        # Handle detection settings form
            ...
            new_enabled_objects = request.form.getlist('enabled_objects') if 'enabled_objects' in request.form else []
        ...
        return jsonify({
            'success': True,
            'message': 'Configuration updated successfully' + (' (system restarted)' if restart_required else ''), 
            'config': CONFIG
        })
